...
extension ModelVersionType {
    ...
    public init?(storeURL: NSURL) {
        guard let metadata = try? NSPersistentStoreCoordinator
            .metadataForPersistentStoreOfType(NSSQLiteStoreType,
                URL: storeURL, options: nil) else
        {
            return nil
        }
        let version = Self.AllVersions.findFirstOccurence {
            $0.managedObjectModel().isConfiguration(nil,
    ...
    public func managedObjectModel() -> NSManagedObjectModel {
        let omoURL = modelBundle.URLForResource(name,
            withExtension: "omo", subdirectory: modelDirectoryName)
        let momURL = modelBundle.URLForResource(name,
    ...
    public func mappingModelsToSuccessor___() -> [NSMappingModel]? {
        guard let nextVersion = successor else { return nil }
        guard let mapping = NSMappingModel(fromBundles: [modelBundle],
    ...
    public func migrationStepsToVersion(version: Self) -> [MigrationStep] {
        ...
        let step = MigrationStep(sourceModel: managedObjectModel(),
            destinationModel: nextVersion.managedObjectModel(),
            mappingModels: mappings)
...
extension NSManagedObjectContext {
    public convenience init<Version: ModelVersionType>(
        concurrencyType: NSManagedObjectContextConcurrencyType,
        modelVersion: Version, storeURL: NSURL, progress: NSProgress? = nil)
    {
        if let storeVersion = Version(storeURL: storeURL)
            where storeVersion != modelVersion
        {
            migrateStoreFromURL(storeURL, toURL: storeURL,
                targetVersion: modelVersion, deleteSource: true,
                progress: progress)
        }
        let psc = NSPersistentStoreCoordinator(
            managedObjectModel: modelVersion.managedObjectModel())
