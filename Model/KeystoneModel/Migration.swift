...
public func migrateStoreFromURL<Version: ModelVersionType>(
    sourceURL: NSURL, toURL: NSURL, targetVersion: Version,
    deleteSource: Bool = false, progress: NSProgress? = nil)
{
    ...
    for step in migrationSteps {
        ...
        for mapping in step.mappingModels {
            try! manager.migrateStoreFromURL(currentURL,
    ...
    try! NSPersistentStoreCoordinator.replaceStoreAtURL(toURL,
        withStoreAtURL: currentURL)
    if (currentURL != sourceURL) {
        NSPersistentStoreCoordinator.destroyStoreAtURL(currentURL)
    }
